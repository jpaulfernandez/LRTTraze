...
namespace IntelligentKioskSample.Views
{
    ...
    public sealed partial class RealTimeDemo : Page, IRealTimeDataProvider
    {
        ...
        private async void ProcessingLoop()
        {
            while (this.isProcessingLoopInProgress)
            {
                await this.Dispatcher.RunAsync(Windows.UI.Core.CoreDispatcherPriority.Normal, async () =>
                {
                    ...
                    {
                        DateTime compare = demographics.StartTime.AddMinutes(3);
                        if (DateTime.Now.Minute > compare.Minute)
                        {
                            // We have been running through the hour. Reset the data...
                            await this.ResetDemographicsData();
                            this.UpdateDemographicsUI();
                            demographics.StartTime = DateTime.Now;
                        }
        ...
        public async Task MongoCreateAsync(Visitor visitor)
        {
            MongoHelper mongo = new MongoHelper();
            if (visitor.Count <= 10)
            {
                visitor.Category = "Low Crowd";
            }
            else if (visitor.Count > 10 && visitor.Count <= 30)
            {
                visitor.Category = "Moderate Crowd";
            }
            else
            {
                visitor.Category = "High Crowd";
            }
            visitor.CapturedDate = DateTime.Now.ToString("dddd, dd MMMM yyyy HH:mm:ss");
            visitor.Location = "doroteo jose";
            visitor.Type = "platform";
            mongo.CreateTask(visitor);
            //await ResetDemographicsData();
        ...
        private void UpdateDemographics(ImageAnalyzer img)
        {
            ...
            if (this.lastSimilarPersistedFaceSample != null)
            {
                ...
                foreach (var item in this.lastSimilarPersistedFaceSample)
                {
                    ...
                    visitor.Count = this.demographics.Visitors.Count;
                    System.Threading.Timer timer = null;
                    timer = new System.Threading.Timer((obj) =>
                    {
                        MongoCreateAsync(visitor);
                        timer.Dispose();
                    },
                                null, 50000, System.Threading.Timeout.Infinite);
